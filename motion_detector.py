...
def detect_motion(frames, frame_idx, threshold=25, min_area=100):
    ...
    blur_current = cv2.GaussianBlur(gray_current, (5, 5), 0)
    blur_prev = cv2.GaussianBlur(gray_prev, (5, 5), 0)

    # 3. Calculate absolute difference between frames (hint: cv2.absdiff)
    diff = cv2.absdiff(blur_prev, blur_current)
    ...
    # motion_boxes = []  ← (already used above)
